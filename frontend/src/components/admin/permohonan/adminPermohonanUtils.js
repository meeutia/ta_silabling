...
const pickSampleIdentity = (sample) => sample?.id_fppl_sampel || sample?.idFpplSampel || sample?.id || '';

const pickSampleTypeId = (sample) =>
  sample?.id_jenis_sampel ||
  sample?.idJenisSampel ||
  sample?.JenisSampel?.id_jenis_sampel ||
  sample?.jenis_sampel?.id_jenis_sampel ||
  sample?.jenisSampel?.idJenisSampel ||
  '';

const pickRegBmId = (sample) =>
  sample?.id_reg_bm ||
  sample?.idRegBm ||
  sample?.RegBm?.id_reg_bm ||
  sample?.reg_bm?.id_reg_bm ||
  sample?.regBm?.idRegBm ||
  '';

const pickParameterIdentity = (sampleParameterMethod) => {
...
const getAllSampleParameterMethods = (requestSample) => [
...
const mergeUniqueByIdentity = (items, pickIdentity) => {
  const map = new Map();
...
const buildSampleBusinessKey = (sample) => {
...
const mergeRequestSample = (existing, incoming) => {
  const existingQuantity = readSampleQuantity(existing);
  const incomingQuantity = readSampleQuantity(incoming);
...
export const getSampleParameterMethods = (requestSample) =>
  mergeUniqueByIdentity(getAllSampleParameterMethods(requestSample), pickParameterIdentity);
