...
function buildContentHtml({ namaPenyelia, sampleNo, catatanRevisi, items, reviewLink }) {
  return `
    <p style="margin:0 0 14px 0;color:#111827;font-size:14px;line-height:1.7;">Yth. ${escapeHtml(namaPenyelia)},</p>
    <p style="margin:0 0 18px 0;color:#111827;font-size:14px;line-height:1.7;">Kasi Pengujian mengajukan revisi hasil pengujian untuk sampel <strong>${escapeHtml(sampleNo)}</strong>.</p>

    <div style="margin:18px 0 8px 0;color:#047857;font-size:12px;line-height:1.5;font-weight:800;text-transform:uppercase;letter-spacing:.05em;">Parameter/metode yang diajukan revisi</div>
    ...
    <div style="margin:18px 0;border:1px solid #e5e7eb;border-radius:16px;background:#f9fafb;padding:14px;">
      <p style="margin:0;color:#111827;font-size:14px;line-height:1.7;">Silakan setujui atau tolak pengajuan revisi tersebut melalui tombol di bawah email ini.</p>
      <p style="margin:8px 0 0 0;color:#6b7280;font-size:12px;line-height:1.6;">Revisi belum diteruskan ke Analis sebelum disetujui oleh Penyelia.</p>
      ${reviewLink ? `<p style="margin:8px 0 0 0;color:#6b7280;font-size:11px;line-height:1.5;word-break:break-word;overflow-wrap:anywhere;">Link cadangan: <a href="${escapeHtml(reviewLink)}" style="color:#047857;font-weight:700;text-decoration:none;">${escapeHtml(reviewLink)}</a></p>` : ''}
    </div>
