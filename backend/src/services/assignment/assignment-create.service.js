...
const { User, FpplParameterMetode, ParameterMetode, Penugasan, PenugasanDetail, PenugasanItem, Sampel, } = require('../../models/Associations');
...
const { pairKey, methodGroupKeyFromFpm, assignmentGroupKey, isInternalCapableFpm, } = require('./assignment-fpm.helper');
...
class AssignmentCreateService {
    ...
    normalizeAssignmentRequestData = (assignments = []) => {
        return assignments
            ...
            const rawNoSampel = Array.isArray(item.no_sampel || item.noSampel)
                ...
                : [];
            const noSampelRows = Array.from(new Set(rawNoSampel
                .map((value) => String(value || '').trim())
                .filter(Boolean)));
            const rawPairs = Array.isArray(item.pairs) ? item.pairs : [];
            const pairs = rawPairs
                .map((pair) => ({
                id_fppl_parameter_metode: String(pair.id_fppl_parameter_metode ||
            ...
            const representativeFpmId = String(item.id_fppl_parameter_metode ||
                item.idFpplParameterMetode ||
                pairs[0]?.id_fppl_parameter_metode ||
                '').trim();
            const normalizedPairs = pairs.length > 0
                ? pairs
            ...
            return {
                id_fppl_parameter_metode: representativeFpmId,
                id_metode_parameter: String(item.id_metode_parameter ||
            ...
            .filter((item) => item.id_fppl_parameter_metode &&
    ...
    createAssignment = async (requestData, currentUserNik) => {
        ...
        const requestedFpmIds = Array.from(new Set(normalizedAssignments.flatMap((item) => [
            item.id_fppl_parameter_metode,
            ...item.pairs.map((pair) => pair.id_fppl_parameter_metode),
        ]))).filter(Boolean);
        const sampleNos = Array.from(new Set(normalizedAssignments.flatMap((item) => item.no_sampel))).filter(Boolean);
        const result = await sequelize.transaction(async (transaction) => {
            ...
            const requestedFpmRows = requestedFpmInstances
                ...
                .filter(Boolean);
            const requestedMethodIds = Array.from(new Set(requestedFpmRows
                .map((fpm) => methodGroupKeyFromFpm(fpm))
                .filter(Boolean)));
            const fpmWhere = requestedMethodIds.length > 0
                ? {
                    id_metode_parameter: {
            ...
            existingAssignedByMethodInstances.forEach((instance) => {
                ...
                items.forEach((item) => {
                    if (!detailMethodId || !item?.no_sampel)
                        return;
            ...
            for (const item of normalizedAssignments) {
                ...
                const explicitPairBySample = new Map(item.pairs.map((pair) => [pair.no_sampel, pair.id_fppl_parameter_metode]));
                ...
                for (const noSampel of item.no_sampel) {
                    ...
                    if (!resolvedFpmId || !validPairSet.has(actualPairKey)) {
                        throw new Error(`Pasangan parameter-sampel tidak valid: ${item.id_fppl_parameter_metode} / ${noSampel}`);
                    }
                    if (resolvedMethodId && resolvedMethodId !== representativeMethodId) {
                        throw new Error(`Sampel ${noSampel} tidak berada pada metode parameter yang sama.`);
                    }
                    if (duplicatePairSet.has(actualPairKey) ||
                        duplicateGroupSet.has(groupDuplicateKey) ||
                        duplicateGroupSet.has(methodDuplicateKey)) {
                        throw new Error(`Sampel ${noSampel} untuk parameter-metode ini sudah pernah ditugaskan.`);
                    }
        ...
        try {
            await this.notificationService.notifyPenugasanAnalisBaru(result.idPenugasan);
        }
        catch (error) {
...
module.exports = new AssignmentCreateService();
module.exports.AssignmentCreateService = AssignmentCreateService;
