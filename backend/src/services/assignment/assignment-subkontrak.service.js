...
const { Pelanggan, Fppl, FpplSampel, RegBm, JenisSampel, Parameter, Metode, ParameterMetode, FpplParameterMetode, Penugasan, PenugasanDetail, PenugasanItem, Sampel, Lka, LkaHasil, } = require('../../models/Associations');
...
const { getPlain, pickObject, pickArray, } = require('./assignment-object.helper');
const { pairKey, assignmentPendingKey, sortSamplesForAssignment, isSubkontrakFpm, } = require('./assignment-fpm.helper');
const { subkontrakAssignmentWhere, } = require('./assignment-scope.helper');
const { getLkaHasilKey, } = require('./assignment-revision.helper');
const { assertSamplesEditableBeforeLhu, } = require('./assignment-lhu-lock.helper');
class AssignmentSubkontrakService {
isValidResultExpression = (value) => {
    ...
    findExistingSubkontrakDetail = async (idMetodeParameter, transaction) => {
    ...
    ensureSubkontrakPenugasan = async (currentUserNik, transaction) => {
    ...
    ensureSubkontrakDetail = async (idMetodeParameter, currentUserNik, transaction, penugasanCache) => {
        if (!idMetodeParameter) {
            throw new Error('ID metode parameter subkontrak tidak ditemukan.');
        }
        const existing = await this.findExistingSubkontrakDetail(idMetodeParameter, transaction);
        ...
        if (!penugasanCache.idPenugasan) {
            penugasanCache.idPenugasan = await this.ensureSubkontrakPenugasan(currentUserNik, transaction);
        }
    ...
    ensurePenugasanItemForSample = async (idPenugasanDetail, noSampel, transaction) => {
    ...
    ensureSubkontrakLka = async (idPenugasanDetail, kodeLka, currentUserNik, tanggalTerimaHasil, transaction) => {
    ...
    upsertLkaHasilSubkontrak = async (kodeLka, noSampel, hasil, transaction) => {
    ...
    getSubkontrakItems = async () => {
        ...
        const methodIds = Array.from(new Set(rows
            ...
            .filter(Boolean)));
        const sampleNos = Array.from(new Set(rows
        ...
        subkontrakDetailInstances.forEach((instance) => {
            ...
            items.forEach((item) => {
                ...
                if (!idMetodeParameter || !noSampel)
                    return;
                const hasilRow = hasilRows.find((hasil) => hasil.no_sampel === noSampel) || {};
                savedByMethodSample.set(assignmentPendingKey(idMetodeParameter, noSampel), {
                    detail,
                    lka,
                    hasilRow,
                });
        ...
        return rows
            ...
            return samples.map((sampel) => {
                const saved = savedByMethodSample.get(assignmentPendingKey(idMetodeParameter, sampel.no_sampel)) || {};
    ...
    saveSubkontrakResults = async (requestData, currentUserNik) => {
        ...
        return sequelize.transaction(async (transaction) => {
            ...
            const fpmById = new Map(fpmRows.map((fpm) => [String(fpm.id_fppl_parameter_metode), fpm]));
            ...
            for (const item of normalizedResults) {
                ...
                if (!this.isValidResultExpression(item.hasil)) {
            ...
            for (const item of normalizedResults) {
                ...
                const detail = await this.ensureSubkontrakDetail(idMetodeParameter, currentUserNik, transaction, penugasanCache);
                await this.ensurePenugasanItemForSample(detail.idPenugasanDetail, item.no_sampel, transaction);
                const kodeLka = await this.ensureSubkontrakLka(detail.idPenugasanDetail, detail.kodeLka, currentUserNik, item.tanggal_terima_hasil, transaction);
                await this.upsertLkaHasilSubkontrak(kodeLka, item.no_sampel, item.hasil, transaction);
                savedRows.push({
                    kodeLka,
            ...
            await Sampel.update({ status_sample: 'Dalam Pengujian' }, {
                where: {
                    no_sampel: { [Op.in]: sampleNos },
                },
                transaction,
...
module.exports = new AssignmentSubkontrakService();
module.exports.AssignmentSubkontrakService = AssignmentSubkontrakService;
