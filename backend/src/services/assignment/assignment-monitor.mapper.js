...
class AssignmentMonitorMapper {
deriveSampleStatus = (row) => {
        ...
        if (assigned === 0)
            return 'Pending Penugasan';
        if (assigned < total)
            return 'Sebagian Ditugaskan';
        if (revisi > 0)
            return 'Perlu Revisi';
        if (worksheetSent > 0)
    ...
    getDetailParameterInfo = (detail = {}) => {
        ...
        const fpmParameterMetode = pickObject(fpm, ['parameter_metode', 'ParameterMetode']) || {};
        const parameterMetode = directParameterMetode.id_metode_parameter
        ...
        return {
            ...
            parameterMetode,
            metode,
            idMetodeParameter: detail.id_metode_parameter ||
    ...
    getDetailSampleRows = (detail = {}) => {
        ...
        return penugasanItems
            ...
            return {
                ...
                no_sampel: noSampel,
                noSampel,
                tanggal_penugasan: detail.penugasan?.assigned_at || detail.Penugasan?.assigned_at || null,
    ...
    getLatestActivityFromDetail = (detail = {}) => {
        const lka = pickObject(detail, ['lka', 'Lka']) || {};
        const sampleRows = this.getDetailSampleRows(detail);
        return (lka.tanggal_pelaporan ||
            lka.tanggal_pemeriksaan ||
            detail.penugasan?.assigned_at ||
            detail.Penugasan?.assigned_at ||
            detail.tanggal_tenggat ||
            null);
    };
    isInternalDetail = (detail = {}) => {
...
module.exports = new AssignmentMonitorMapper();
module.exports.AssignmentMonitorMapper = AssignmentMonitorMapper;
