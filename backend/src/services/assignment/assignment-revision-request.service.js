...
const { Sampel, Lka, PenugasanDetail, } = require('../../models/Associations');
const { SAMPLE_REVIEW_STATUS, } = require('../../constants/lhu-status.constant');
const { prefixRevisionNote, buildRevisionNotePatch, normalizeRevisionRole, } = require('./assignment-revision.helper');
const { syncAssignmentHeaderStatusForDetails, } = require('./assignment-status.helper');
const { assertSamplesEditableBeforeLhu, cancelNonFinalLhusBySamples, } = require('./assignment-lhu-lock.helper');
const { assertSampleReadyForKasiReview, } = require('./assignment-kasi-review.service');
const { getLkaResultRowsForSample, } = require('./assignment-lka-result.service');
class AssignmentRevisionRequestService {
requestLkaRevision = async ({ noSampel, sourceRole, catatanRevisi, currentNik }) => {
        ...
        return sequelize.transaction(async (transaction) => {
            ...
            const affectedRows = Array.isArray(lkaRows)
                ...
                : [];
            const affectedLkaCodes = Array.from(new Set(affectedRows.map((row) => row.kode_lka).filter(Boolean)));
            const affectedDetailIds = Array.from(new Set(affectedRows.map((row) => row.id_penugasan_detail).filter(Boolean)));
            ...
            if (affectedDetailIds.length > 0) {
                await PenugasanDetail.update({ status_detail: 'Perlu Revisi' }, {
                    where: {
...
module.exports = new AssignmentRevisionRequestService();
module.exports.AssignmentRevisionRequestService = AssignmentRevisionRequestService;
