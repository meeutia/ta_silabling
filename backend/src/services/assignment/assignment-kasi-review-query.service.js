...
const { resolveLkaHasilStatus, } = require('./assignment-status.helper');
...
getBmParamMap = async (sample = {}) => {
    ...
    getCompletedKasiResultRowsFromSample = async (sample = {}, transaction = null) => {
        const noSampel = sample.no_sampel;
        const fpms = getAssociatedFpmsFromSample(sample);
        const rows = [];
        const bmMap = await this.getBmParamMap(sample);
        ...
        return rows.sort((a, b) => String(a.nama_parameter || '').localeCompare(String(b.nama_parameter || '')));
    };
    countKasiReviewCompletion = async (sample = {}) => {
        const fpms = getAssociatedFpmsFromSample(sample);
        const totalParameter = fpms.length;
        const completedRows = await this.getCompletedKasiResultRowsFromSample(sample);
    ...
    mapKasiReviewSampleHeader = (sample = {}) => {
        const fpplSampel = pickObject(sample, ['fppl_sampel', 'FpplSampel']) || {};
    ...
    getKasiReviewSampleHeader = async (noSampel, transaction = null) => {
        const sampleInstance = await this.findKasiReviewSample(noSampel, transaction);
        if (!sampleInstance) {
            throw new Error('Sampel tidak ditemukan.');
        }
        return this.mapKasiReviewSampleHeader(getPlain(sampleInstance));
    };
    getKasiReviewResultRows = async (noSampel, transaction = null) => {
        const sampleInstance = await this.findKasiReviewSample(noSampel, transaction);
        if (!sampleInstance) {
            throw new Error('Sampel tidak ditemukan.');
        }
        return this.getCompletedKasiResultRowsFromSample(getPlain(sampleInstance), transaction);
    };
}
module.exports = new AssignmentKasiReviewQueryService();
module.exports.AssignmentKasiReviewQueryService = AssignmentKasiReviewQueryService;
