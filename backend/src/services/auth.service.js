...
class AuthService {
generateToken = (user) => {
        return jwt.sign({ nik: user.nik, id_role: user.id_role }, process.env.JWT_SECRET, { expiresIn: ACCESS_TOKEN_EXPIRES_IN });
    };
    ...
    getPelanggansFromUser = (user) => {
        if (Array.isArray(user?.pelanggans))
            return user.pelanggans;
        if (Array.isArray(user?.Pelanggans))
            return user.Pelanggans;
        if (Array.isArray(user?.pelanggan))
    ...
    findUserWithProfile = async (where) => {
        return User.findOne({
            where,
            include: [
                {
    ...
    login = async (identifier, password) => {
        ...
        if (!user) {
            throw new Error(`Username/email atas nama "${normalizedIdentifier}" tidak terdaftar. Silakan melakukan this.register.`);
        }
        if (!user.is_active) {
    ...
    forgotPassword = async (email) => {
        ...
        const expiresAt = this.getResetPasswordExpiryDate();
        await user.update({
        ...
        const safeUsername = this.escapeHtml(user.username || user.email || 'Pengguna');
        await sendMail({
            to: user.email,
            subject: 'Reset Kata Sandi Akun SILABLING',
            text: `Halo ${user.username || 'Pengguna'},\n\n` +
    ...
    refresh = async (incomingRefreshToken) => {
        ...
        const refreshTokenHash = this.hashRefreshToken(incomingRefreshToken);
        const user = await this.findUserWithProfile({ refresh_token_hash: refreshTokenHash });
        if (!user) {
    ...
    getMe = async (nik) => {
        const user = await this.findUserWithProfile({ nik });
        if (!user) {
