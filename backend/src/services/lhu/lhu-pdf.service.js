...
const LHU_PDF_OWNER_PASSWORD = process.env.LHU_PDF_OWNER_PASSWORD || 'SILABLING-LHU-OWNER-CHANGE-ME';
...
findExistingFile = (paths = []) => {
    ...
    registerFonts = (doc) => {
        const regularPath = this.findExistingFile(FONT_CANDIDATES.regular);
        const boldPath = this.findExistingFile(FONT_CANDIDATES.bold);
        if (regularPath) {
            doc.registerFont('CenturyGothic', regularPath);
            PDF_FONT.regular = 'CenturyGothic';
        }
        else {
            console.warn('[LHU PDF] Font Century Gothic regular tidak ditemukan. ' +
        ...
        if (boldPath) {
            ...
            console.warn('[LHU PDF] Font Century Gothic bold tidak ditemukan. ' +
                'Letakkan CenturyGothic-Bold.ttf di public/assets/fonts atau isi LHU_FONT_BOLD_PATH.');
        }
    };
    drawHeader = (doc, lhu, isFinal, accreditationStats = {}, options = {}) => {
        const startY = 28;
        const logoSize = 58;
        ...
        doc.font(PDF_FONT.bold).fontSize(11).text('PEMERINTAH PROVINSI SUMATERA BARAT', headerX, headerY, {
            width: headerWidth,
            align: 'center',
        });
        doc.font(PDF_FONT.bold).fontSize(10.5).text('DINAS LINGKUNGAN HIDUP', headerX, doc.y + 1, {
            width: headerWidth,
            align: 'center',
        });
        doc.font(PDF_FONT.bold).fontSize(10.5).text('UPTD LABORATORIUM LINGKUNGAN', headerX, doc.y + 1, {
            width: headerWidth,
            align: 'center',
        });
        doc.font(PDF_FONT.regular).fontSize(7).text('Registrasi Kompetensi Laboratorium Lingkungan Nomor : 00274/LP/R/ABLING-1/LK/KLH', headerX, doc.y + 3, {
            width: headerWidth,
            align: 'center',
        });
        doc.fontSize(7).text('Jl. Khatib Sulaiman No. 22 Telp. (0751) 7055231 - 446571 - 445154  Fax. (0751) 449232 Padang', headerX, doc.y + 1, {
            width: headerWidth,
            align: 'center',
        });
        doc.fontSize(7).text('Website: http://dlh.sumbarprov.go.id  Email: lablingprovsumabar@gmail.com', headerX, doc.y + 1, {
            width: headerWidth,
            align: 'center',
        });
    ...
    buildParameterGroups = (details = []) => {
        const map = new Map();
        details.forEach((row) => {
            const key = this.buildParameterGroupKey(row);
            if (!map.has(key)) {
                map.set(key, {
                    ...row,
                    hasil_by_sample: {},
    ...
    drawTableHeader = (doc, y, sampleChunk = []) => {
        const columns = this.getTableColumns(sampleChunk);
        const topHeight = 17;
    ...
    getCellTextHeight = (doc, text, width, align = 'left') => {
    ...
    getRowHeight = (doc, row, sampleChunk = []) => {
        const columns = this.getTableColumns(sampleChunk);
    ...
    startContinuationPage = (doc, lhu, isFinal, accreditationStats = {}) => {
        doc.addPage();
        this.drawDraftWatermark(doc, isFinal);
        this.drawHeader(doc, lhu, isFinal, accreditationStats, { includeDocumentHeader: false });
    };
    drawDetailTable = (doc, details, isFinal, lhu = {}, accreditationStats = {}) => {
        ...
        sampleChunks.forEach((sampleChunk, chunkIndex) => {
            let globalIndex = 1;
            if (chunkIndex > 0) {
                this.startContinuationPage(doc, lhu, isFinal, accreditationStats);
            }
            let y = doc.y;
            this.drawTableHeader(doc, y, sampleChunk);
            y += TABLE_HEADER_HEIGHT;
            doc.font(PDF_FONT.regular).fontSize(TABLE_FONT_SIZE);
            parameterGroups.forEach((row) => {
                const rowHeight = this.getRowHeight(doc, row, sampleChunk);
                if (y + rowHeight > 735) {
                    this.startContinuationPage(doc, lhu, isFinal, accreditationStats);
                    y = doc.y;
                    this.drawTableHeader(doc, y, sampleChunk);
                ...
                const values = columns.map((col) => {
                    if (col.type === 'no')
                        return { ...col, text: String(globalIndex) };
                    if (col.type === 'parameter')
                        return { ...col, text: this.buildParameterName(row) };
                    if (col.type === 'hasil')
                        return { ...col, text: valueOrDash(row.hasil_by_sample?.[col.sampleNo]) };
                ...
                values.forEach((col) => {
                    if (col.x > TABLE_LEFT)
                        doc.lineWidth(TABLE_LINE_WIDTH).moveTo(col.x, y).lineTo(col.x, y + rowHeight).stroke('#111111');
    ...
    getNormalBottomY = (doc) => {
        return doc.page.height - NORMAL_BOTTOM_MARGIN;
    };
    getHardBottomY = (doc) => {
        return doc.page.height - HARD_BOTTOM_MARGIN;
    };
    measureTextHeight = (doc, text, width, fontSize = NOTES_FONT_SIZE, lineGap = NOTES_LINE_GAP) => {
    ...
    measureNotesHeight = (doc, lhu, details = [], isFinal, totalPages = 1) => {
        const notes = this.buildLhuNotes(lhu, details, isFinal, totalPages);
        ...
        height += 12;
        height += Math.max(12, this.measureTextHeight(doc, coordinateText, coordinateValueWidth, 7.5, NOTES_LINE_GAP));
        height += 6;
        height += 11;
        notes.forEach((note) => {
            height += Math.max(this.measureTextHeight(doc, note, CONTENT_WIDTH - 28), this.measureTextHeight(doc, '1.', 18)) + 2;
        });
        return Math.ceil(height + 4);
    };
    drawNotes = (doc, lhu, details = [], isFinal, totalPages = 1) => {
        let y = doc.y;
        const sectionColonX = PAGE_LEFT + 92;
        const coordinateLabelX = PAGE_LEFT + 22;
        const coordinateColonX = sectionColonX;
        ...
        y += 12;
        const coordinateHeight = Math.max(12, doc.heightOfString(coordinateText, {
        ...
        notes.forEach((note, index) => {
            const noteHeight = Math.max(doc.heightOfString(`${index + 1}.`, { width: 18, lineGap: NOTES_LINE_GAP }), doc.heightOfString(note, { width: CONTENT_WIDTH - 28, align: 'left', lineGap: NOTES_LINE_GAP }));
    ...
    drawClosingSection = (doc, lhu, details = [], isFinal) => {
        ...
        let notesHeight = this.measureNotesHeight(doc, lhu, details, isFinal, totalPages);
        let requiredHeight = notesHeight + SIGNATURE_GAP + SIGNATURE_HEIGHT + 8;
        if (doc.y + requiredHeight > this.getHardBottomY(doc)) {
            doc.addPage();
            this.drawDraftWatermark(doc, isFinal);
            this.drawHeader(doc, lhu, isFinal, calculateAccreditationStats(details), { includeDocumentHeader: false });
            totalPages = pageCountBeforeClosing + 1;
            notesHeight = this.measureNotesHeight(doc, lhu, details, isFinal, totalPages);
            requiredHeight = notesHeight + SIGNATURE_GAP + SIGNATURE_HEIGHT + 8;
        }
        this.drawNotes(doc, lhu, details, isFinal, totalPages);
        let signatureY = doc.y + SIGNATURE_GAP;
        if (signatureY + SIGNATURE_HEIGHT > this.getNormalBottomY(doc)) {
            signatureY = Math.max(doc.y + 6, this.getHardBottomY(doc) - SIGNATURE_HEIGHT);
        }
        if (signatureY + SIGNATURE_HEIGHT > this.getHardBottomY(doc)) {
            signatureY = doc.y + 6;
        }
        this.drawSignature(doc, lhu, isFinal, signatureY);
    };
    ...
    generateLhuPdf = async (nomorLhu, options = {}) => {
        ...
        await new Promise((resolve, reject) => {
            ...
            this.drawDraftWatermark(doc, isFinal);
            this.drawHeader(doc, lhu, isFinal, accreditationStats, { includeDocumentHeader: false });
            this.drawInfoSection(doc, lhu);
            this.drawDetailTable(doc, details, isFinal, lhu, accreditationStats);
            this.drawClosingSection(doc, lhu, details, isFinal);
            doc.end();
        });
        return {
            filePath: publicPath,
...
module.exports = new LhuPdfService();
module.exports.LhuPdfService = LhuPdfService;
