...
} = require('./payment-xendit.service');
...
} = require('./payment-session-payload.util');
...
const createGatewayPayment = async (requestId, userNik, paymentMethodCode) => {
  ...
  try {
    ...
    if (requestRecord.status_fppl !== RequestStatus.WAITING_PAYMENT) {
      throw new Error(
        `Permohonan tidak bisa diproses ke payment gateway karena status saat ini: ${requestRecord.status_fppl}`
      );
    }
...
function notifyPaymentCompletedInBackground(idRegistrasi) {
  const registrasiId = String(idRegistrasi || '').trim();
  if (!registrasiId) return;

  setImmediate(() => {
    const notificationService = require('../notification/notification-request.service');

    Promise.allSettled([
      notificationService.notifyPaymentCompletedToAdmin({ idRegistrasi: registrasiId }),
      notificationService.notifyPaymentCompletedToCustomer({ idRegistrasi: registrasiId }),
    ]).then((results) => {
...
function triggerPaymentNotificationFromResult(result = {}) {
  if (result?.notificationContext?.event !== 'PAYMENT_COMPLETED') return;
  notifyPaymentCompletedInBackground(result.notificationContext.idRegistrasi);
}

const extractXenditPaymentId = (data = {}) => {
  const payment = firstArrayItem(data.payments) || data.payment || {};
  const paymentRequest = firstArrayItem(data.payment_requests) || data.payment_request || {};
...
const extractXenditPaymentRequestId = (data = {}) => {
  const paymentRequest = firstArrayItem(data.payment_requests) || data.payment_request || {};
  const payment = firstArrayItem(data.payments) || data.payment || {};
...
const extractXenditPaidAt = (data = {}, payload = {}) => {
  const payment = firstArrayItem(data.payments) || data.payment || {};
...
async function applyXenditPaymentSessionUpdate(payload, transaction) {
  ...
  await payment.update(
    {
      ...
      gateway_payment_id: extractXenditPaymentId(data) || payment.gateway_payment_id || null,
      gateway_payment_request_id: extractXenditPaymentRequestId(data) || payment.gateway_payment_request_id || null,
  ...
  if (status === 'COMPLETED') {
    ...
    const paidAt = extractXenditPaidAt(data, payload);
...
async function syncXenditPaymentStatusFromReturn(query = {}) {
  ...
  try {
    const result = await applyXenditPaymentSessionUpdate(xenditSession, t);
    await t.commit();
...
const handleXenditPaymentSessionWebhook = async (req, res) => {
  ...
  try {
    const result = await applyXenditPaymentSessionUpdate(payload, t);
