...
class PaymentSessionRequestDataUtil {
buildPaymentGatewayRequestData = (payment) => {
        ...
        return {
            ...
            amount: normalizeAmount(payment.gatewayData?.amount || 0),
            isXendit: payment.gateway_provider === 'XENDIT'
        };
    };
    normalizePhoneForXendit = (phone) => {
    ...
    getPublicBackendBaseUrl = () => {
        return this.trimTrailingSlash(process.env.PUBLIC_BACKEND_URL ||
            process.env.BACKEND_PUBLIC_URL ||
            process.env.APP_URL ||
            '');
    };
    buildBackendPaymentBridgeUrl = (status) => {
        const backendUrl = this.getPublicBackendBaseUrl();
        if (!backendUrl)
            return null;
    ...
    normalizeUrlWithPaymentParams = (rawUrl, params = {}) => {
    ...
    buildXenditReturnUrl = ({ status, requestId, invoiceId, paymentId }) => {
        const envName = status === 'success' ? 'XENDIT_SUCCESS_RETURN_URL' : 'XENDIT_CANCEL_RETURN_URL';
        const configuredUrl = this.parseHttpsUrlForXendit(process.env[envName]);
        const backendBridgeUrl = this.parseHttpsUrlForXendit(this.buildBackendPaymentBridgeUrl(status));
        const baseUrl = configuredUrl || backendBridgeUrl;
        if (!baseUrl) {
            throw new Error(`${envName} atau PUBLIC_BACKEND_URL harus berupa URL HTTPS valid. ` +
        ...
        return this.normalizeUrlWithPaymentParams(baseUrl, {
            id_registrasi: requestId,
            id_invoice: invoiceId,
            id_payment: paymentId,
        });
    };
    buildFrontendPaymentStatusUrl = ({ requestId, invoiceId, paymentId, status }) => {
        const frontendUrl = this.getFrontendBaseUrl();
        return this.normalizeUrlWithPaymentParams(`${frontendUrl}/pelanggan/status`, {
            payment: status,
            id_registrasi: requestId,
    ...
    buildPaymentReturnUrls = ({ requestId, invoiceId, paymentId }) => {
        return ({
            success_return_url: this.buildXenditReturnUrl({ status: 'success', requestId, invoiceId, paymentId }),
            cancel_return_url: this.buildXenditReturnUrl({ status: 'failed', requestId, invoiceId, paymentId })
        });
    };
    buildXenditPaymentSessionRequestData = ({ requestJson, requestId, invoice, payment, amount, referenceId, paymentMethod = null }) => {
        ...
        const requestData = {
            ...
            customer: {
                ...
                mobile_number: this.normalizePhoneForXendit(pelanggan.no_telp),
                individual_detail: {
        ...
        if (paymentMethod && paymentMethod.channel && !['hosted_checkout', 'bayar_nanti'].includes(paymentMethod.channel)) {
            requestData.allowed_payment_channels = [paymentMethod.channel];
        }
        return requestData;
    };
}
const paymentSessionRequestDataUtil = new PaymentSessionRequestDataUtil();

// Backward-compatible aliases for existing service imports.
// Keep the canonical method names above, but expose the shorter names used by payment.service.js.
paymentSessionRequestDataUtil.buildPaymentGatewayData = paymentSessionRequestDataUtil.buildPaymentGatewayRequestData;
paymentSessionRequestDataUtil.buildXenditPaymentSessionData = paymentSessionRequestDataUtil.buildXenditPaymentSessionRequestData;

module.exports = paymentSessionRequestDataUtil;
module.exports.PaymentSessionRequestDataUtil = PaymentSessionRequestDataUtil;
