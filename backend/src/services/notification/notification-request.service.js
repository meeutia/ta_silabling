...
} = require('../../templates/email/payment-customer-action.template');


async function getLatestInvoiceAndPayment(registrasiId) {
...
async function createEmailLogIfNotAlreadyQueuedOrSent({
  idTipeNotifikasi,
...
async function notifyPaymentCompletedToAdmin({ idRegistrasi } = {}) {
  const registrasiId = safeString(idRegistrasi).trim();
  ...
  const tipe = await findOrCreateNotificationTypeById(
    NOTIFICATION_TYPE.PEMBAYARAN_PELANGGAN_ADMIN,
    {
  ...
  const { invoice, payment } = await getLatestInvoiceAndPayment(registrasiId);
  const recipients = await getActiveUsersByRole(Roles.ADMIN);
  const results = [];
  ...
  for (const penerima of recipients) {
    ...
    const logResult = await createEmailLogIfNotAlreadyQueuedOrSent({
      idTipeNotifikasi: tipe.get('id_tipe_notifikasi'),
      penerimaUserNik: nik,
      penerimaPelangganId: null,
      idRegistrasi: registrasiId,
...
async function notifyPaymentCompletedToCustomer({ idRegistrasi } = {}) {
  const registrasiId = safeString(idRegistrasi).trim();
  ...
  const tipe = await findOrCreateNotificationTypeById(
    NOTIFICATION_TYPE.PEMBAYARAN_TERVERIFIKASI,
    {
  ...
  if (logResult.skipped) return logResult;
...
async function notifyCustomerRequestCancelledToAdmin({ idRegistrasi, note = null } = {}) {
  const registrasiId = safeString(idRegistrasi).trim();
  ...
  const tipe = await findOrCreateNotificationTypeById(
    NOTIFICATION_TYPE.PEMBATALAN_PELANGGAN_ADMIN,
    {
...
async function notifyCustomerRequestCancelledToCustomer({ idRegistrasi, note = null } = {}) {
  const registrasiId = safeString(idRegistrasi).trim();
  ...
  const tipe = await findOrCreateNotificationTypeById(
    NOTIFICATION_TYPE.PERMOHONAN_DITOLAK,
    {
...
module.exports = {
  ...
  notifyCustomerRequestCancelledToAdmin,
  notifyCustomerRequestCancelledToCustomer,
  notifyPaymentCompletedToAdmin,
  notifyPaymentCompletedToCustomer,
