...
const { JadwalPengambilanLhu, JadwalSampel, Fppl, Pelanggan, User, Pegawai, PengajuanPerubahanJadwal, } = require('../../models/Associations');
const { NOTIFICATION_TYPE } = require('../../constants/notification.constant');
const Roles = require('../../constants/roles');
const { buildAdminRequestLink, buildRequestDetailLink, safeString, } = require('./notification-format.util');
const { createEmailLog, findNotificationTypeById, findOrCreateNotificationTypeById, getPlain, markEmailFailed, markEmailSent, resolveRecipientEmail, sendNotificationEmail, } = require('./notification-core.service');
...
getActiveUsersByRole = async (roleId) => {
    ...
    notifyJadwalPengambilanLhu = async (idJadwalLhu) => {
        ...
        const tipe = await findNotificationTypeById(NOTIFICATION_TYPE.JADWAL_PENGAMBILAN_LHU);
        ...
        const penerimaPelangganId = pelanggan.id_pelanggan ||
            fppl.id_pelanggan ||
            null;
    ...
    getScheduleChangeNotificationContext = async (idPengajuanJadwal) => {
    ...
    getScheduleChangeNotificationType = async (idTipeNotifikasi, deskripsi) => {
        return findOrCreateNotificationTypeById(idTipeNotifikasi, {
            deskripsi,
            konteks: 'JADWAL',
        });
    };
    notifyScheduleChangeSubmittedToAdmin = async ({ idPengajuanJadwal } = {}) => {
        const tipe = await this.getScheduleChangeNotificationType(NOTIFICATION_TYPE.SCHEDULE_CHANGE_SUBMITTED, 'Pelanggan mengajukan perubahan jadwal');
        const context = await this.getScheduleChangeNotificationContext(idPengajuanJadwal);
        const admins = await this.getActiveUsersByRole(Roles.ADMIN);
        const results = [];
    ...
    notifyScheduleChangeApprovedToCustomer = async ({ idPengajuanJadwal } = {}) => {
        const tipe = await this.getScheduleChangeNotificationType(NOTIFICATION_TYPE.SCHEDULE_CHANGE_APPROVED, 'Pengajuan perubahan jadwal disetujui admin');
        const context = await this.getScheduleChangeNotificationContext(idPengajuanJadwal);
        const log = await createEmailLog({
            idTipeNotifikasi: tipe.get('id_tipe_notifikasi'),
            penerimaUserNik: null,
    ...
    notifyScheduleChangeRejectedToCustomer = async ({ idPengajuanJadwal } = {}) => {
        const tipe = await this.getScheduleChangeNotificationType(NOTIFICATION_TYPE.SCHEDULE_CHANGE_REJECTED, 'Pengajuan perubahan jadwal ditolak admin');
        const context = await this.getScheduleChangeNotificationContext(idPengajuanJadwal);
        const log = await createEmailLog({
            idTipeNotifikasi: tipe.get('id_tipe_notifikasi'),
            penerimaUserNik: null,
    ...
    notifyJadwalSampel = async ({ idRegistrasi, idJadwal = null } = {}) => {
        const registrasiId = safeString(idRegistrasi).trim();
        if (!registrasiId) {
        ...
        const { request, pelanggan, pelangganId } = await this.getRequestAndCustomer(registrasiId);
...
module.exports = new NotificationScheduleService();
module.exports.NotificationScheduleService = NotificationScheduleService;
