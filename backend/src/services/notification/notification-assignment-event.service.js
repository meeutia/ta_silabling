...
notifyDeadlineAnalisDekat = async ({ daysAhead = 2 } = {}) => {
    ...
    notifyPenyeliaApproveKeKasi = async (idPenugasanDetail) => {
        ...
        const registrasiIds = await this.getRegistrasiIdsForPenugasanDetail(detailId);
        ...
        for (const idRegistrasi of registrasiIds) {
            const readiness = await this.getKasiReadinessByRegistrasi(idRegistrasi);
            const fppl = await this.getFpplNotificationInfo(idRegistrasi);
            ...
            const revisionContext = await this.getKasiRevisionReadyForKasiContext(idRegistrasi);
            ...
            for (const instance of recipients) {
                ...
                try {
                    ...
                    console.error(isRevisionReturn
                        ...
                        : 'Gagal kirim notifikasi permohonan siap review ke Kasi Pengujian:', error);
                }
            }
        }
        return results;
